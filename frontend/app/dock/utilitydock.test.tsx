...
const Harness = vi.hoisted(() => ({
    values: {} as Record<string, any>, width: 800,
    column: vi.fn(), tool: vi.fn(), commit: vi.fn(), visible: vi.fn(), collapse: vi.fn(),
}));
vi.mock("react", async (load) => ({
    ...await load<typeof import("react")>(), memo: (fn: any) => fn,
    useState: (initial: any) => [typeof initial === "number" ? Harness.width : typeof initial === "function" ? initial() : initial, vi.fn()],
    useRef: () => ({ current: null }), useCallback: (fn: any) => fn, useEffect: vi.fn(),
}));
vi.mock("jotai", () => ({ useAtomValue: (key: string) => Harness.values[key] }));
vi.mock("./dock-model", async (load) => ({
    ...await load<typeof import("./dock-model")>(),
    DockModel: { getInstance: () => ({
        activeToolAtom: "active", collapsedAtom: "collapsed", columnWidthAtom: "column", toolWidthAtom: "tool",
        setColumnWidth: Harness.column, setToolWidth: Harness.tool, commitPersist: Harness.commit,
        collapse: Harness.collapse, toggle: vi.fn(),
    }) },
}));
...
vi.mock("@/app/aipanel/waveai-model", () => ({ WaveAIModel: { getInstance: () => ({ currentAIMode: "mode", aiModeConfigs: "configs" }) } }));
vi.mock("@/app/workspace/workspace-layout-model", () => ({ WorkspaceLayoutModel: { getInstance: () => ({
    panelVisibleAtom: "chat", getAIPanelVisible: () => Harness.values.chat, setAIPanelVisible: Harness.visible,
}) } }));
...
vi.mock("./crowe-icons", () => Object.fromEntries(["Assistant", "Close", "Hyphae", "Moon", "Network", "Nib", "Rings", "Spore", "Sun", "Vitals"].map((name) => [name + "Icon", "svg"])));
vi.mock("./designreview-model", () => ({ DesignReviewModel: {} }));
vi.mock("./vcs-model", () => ({ VcsModel: {} }));
vi.mock("./dockpanels", () => Object.fromEntries(["Design", "Model", "Mycelium", "Telemetry", "Thinking", "Vcs"].map((name) => [name + "Panel", "tool-panel"])));
...
function render() { return nodes((UtilityDock as any)()); }
function key(key: string) { return { key, shiftKey: false, preventDefault: vi.fn(), stopPropagation: vi.fn() }; }

describe("Dock interaction wiring", () => {
    beforeEach(() => {
        vi.clearAllMocks();
        Harness.width = 800;
        Harness.values = { active: "model", collapsed: false, column: 760, tool: 480, chat: true, mode: "example", configs: {} };
    });
    ...
    it("retains one unkeyed operator subtree when hidden or in compact mode", () => {
        for (const [chat, width] of [[true, 1200], [false, 1200], [true, 700]] as const) {
